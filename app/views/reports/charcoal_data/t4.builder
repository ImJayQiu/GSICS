...
xml.chart( 
  ...
  xml.dataset(
    ...
    :areaBorderColor => "#E366FF"
  )do
    
    xml.set(:value=>"0")
    @supplier_period.each do |c|
      xml.set(:value=>"#{c.t4 rescue nil}")
    end
    xml.set(:value=>"0")
  end
  ...
  xml.trendlines()do
    ...
    xml.line(
      :startValue=>"#{@supplier_period.average(:t4).round(2) rescue nil}",
      :endValue=>"#{@supplier_period.average(:t4).round(2) rescue nil}",
      :color=>"#31B404",
      :displayValue=>"Avg.",
      :thickness=>"1",
      :alpha=>"100"
